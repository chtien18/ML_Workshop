...
def dnn2_training(D1,D2,D3,DO1,DO2,DO3,RSN,mape_record):
    ...
    model.fit(x_train,y_train,validation_data=(x_val,y_val),verbose=0,epochs=10)
...
def position_initialization(N):
    ...
    for i in range(N):
        ...
        initial_position[i,2] = myround(D_min + random.uniform(0, 1)*(D_max-D_min))
        initial_position[i,3] = myround_2(DO_min + random.uniform(0, 1)*(DO_max-DO_min))
        initial_position[i,4] = myround_2(DO_min + random.uniform(0, 1)*(DO_max-DO_min))
        initial_position[i,5] = myround_2(DO_min + random.uniform(0, 1)*(DO_max-DO_min))
        initial_position[i,6] = myround(RSN_min + random.uniform(0, 1)*(RSN_max-RSN_min))
...
def boundary_handling(X):
    ...
    for i in range(N):
        ...
        if X[i,2] < D_min:
            X[i,2] = myround(D_min + random.uniform(0, 1)*(D_max-D_min))
        elif X[i,2] > D_max:
            X[i,2] = myround(D_min + random.uniform(0, 1)*(D_max-D_min))
        ...
        if X[i,4] < DO_min:
            X[i,4] = myround_2(DO_min + random.uniform(0, 1)*(DO_max-DO_min))
        elif X[i,4] > DO_max:
            X[i,4] = myround_2(DO_min + random.uniform(0, 1)*(DO_max-DO_min))
        
        if X[i,5] < DO_min:
            X[i,5] = myround_2(DO_min + random.uniform(0, 1)*(DO_max-DO_min))
        elif X[i,5] > DO_max:
            X[i,5] = myround_2(DO_min + random.uniform(0, 1)*(DO_max-DO_min))
        
        if X[i,6] < RSN_min:
            X[i,6] = myround(RSN_min + random.uniform(0, 1)*(RSN_max-RSN_min))
        elif X[i,6] > RSN_max:
            X[i,6] = myround(RSN_min + random.uniform(0, 1)*(RSN_max-RSN_min))
    return X
...
def pso_opt(N,Max_iter):
    ...
    while I < Max_iter:
        ...
        for i in range(len(X)):
            ...
            fitness_new[i], mape_record = dnn2_training(D1,D2,D3,DO1,DO2,DO3,RSN,mape_record)
        ...
        for i in range(len(X)):
            ...
            X[i,4] = myround_2(X[i,4])
            X[i,5] = myround_2(X[i,5])
            X[i,6] = myround(X[i,6])
